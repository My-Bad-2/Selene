...
static void stdout_callback(struct LogEvent *event)
{
...
static void stream_callback(struct LogEvent *event)
{
...
static void init_log_event(struct LogEvent *event, FILE *stream) { event->stream = stream; }
...
void log_log(int level, const char *file, int line, const char *fmt, ...)
{
    ...
    if (level == LOG_PANIC) { arch_halt(false); }
}
...
static const unsigned char states[]['z' - 'A' + 1] = {
    {
        ...
        S('X') = UINT,
#ifdef USE_FPU
        S('e') = DBL,   S('f') = DBL,   S('g') = DBL,   S('a') = DBL,  S('E') = DBL,
        S('F') = DBL,   S('G') = DBL,   S('A') = DBL,
#endif  // USE_FPU
        S('c') = INT,   S('C') = UINT,  S('s') = PTR,   S('S') = PTR,  S('p') = UIPTR,
        S('n') = PTR,   S('m') = NOARG, S('l') = LPRE,  S('h') = HPRE, S('L') = BIGLPRE,
...
static void out(FILE *stream, const char *str, size_t length) {
  if (!ferror(stream)) {
    __fwrite((const void *)str, length, stream);
  }
}
...
static int printf_core(FILE *stream, const char *fmt, va_list *ap, union arg *nl_arg,
                       int *nl_type) {
